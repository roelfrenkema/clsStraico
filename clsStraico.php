...
Input   : A text you want to hav judged<br>
Output  : an opinion<br>
Purpose : Get a second opinion of the text<br>
<br>
Remarks:<br>
<br>
...
you are looking for an opinion in another language.
	*/
	...
	*/
     private function changeModel($input){
		...
		$intPoint = intval(substr($input,10));
		$this->aiModel = $this->arModels['data'][$intPoint - 1]['model'];
		echo "Model has been set to: ".$this->aiModel."\n";
...
Alternatively use one of the following internal commands.
  ...
  Available commands:
	...
	/smallblog <subject>        - Write blog on subject of 300 words.
	/mediumblog <subject>       - Write blog on subject of 600 words.
	/bigblog <subject>          - Write blog on subject of 1000 words.
...
using _PAGE_ as a placeholder
    ...
    WARNING: This is a work in progress and has no 
	...
	*/
	...
	public function userPrompt() {
		...
		}elseif( substr($input,0,11) == "/completion"){
			$this->debugCompletion();
		}elseif( substr($input,0,10) == "/internals"){
			$this->debugInternals();
		}elseif( substr($input,0,6) == "/price"){
			$this->debugprice();
		}elseif( substr($input,0,6) == "/words"){
			$this->debugwords();
		}elseif( substr($input,0,8) == "/enhance"){
			$this->agentEnhancePrompt(substr($input,9));
		}elseif( substr($input,0,6) == "/ascii"){
			$this->agentAsciiArt(substr($input,7));
